window.addEventListener("load", () => {
    ["iife", "esm"].map((type) => {
        document.getElementById(`${type}-copy-btn`).onclick = function () {
            (async (element) => {
                element.select();
                try {
            ...
            })(document.getElementById(`${type}-url-input`));
        };

        ((element) => {
            element.addEventListener("keyup", function () {
                this.value = `https://cdn.jsdelivr.net/gh/patelka2211/sharer/bundle/sharer.${type}.min.js`;
            });
        })(document.getElementById(`${type}-url-input`));
    });
});
